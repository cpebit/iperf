...
iperf_connect(struct iperf_test *test)
{
    ...
    if (test->ctrl_sck < 0)
	// Create the control channel using an ephemeral por
#if defined(__ANDROID__)
	test->ctrl_sck = netdial(test->settings->domain, Ptcp, test->bind_address, test->bind_dev, 0, test->server_hostname, test->server_port, test->settings->connect_timeout, test->network);
#else
